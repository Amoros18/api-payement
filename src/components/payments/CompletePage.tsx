...
export default function CompletePage() {
  ...
  useEffect(() => {
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      ...
      setStatus(STATUS_CONTENT_MAP[status] ? status : 'default');
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white shadow-md rounded-lg p-6 w-full max-w-lg text-center">
        ...
        <h2 className="text-2xl font-bold mb-4">{STATUS_CONTENT_MAP[status].text}</h2>
        {intentId && (
          <div className="mb-4">
        ...
        <Link
          href="/"
          className="mt-4 inline-block bg-blue-500 text-white px-4 py-2 rounded-lg transition duration-300 ease-in-out hover:bg-blue-700"
        >
          Test another
        </Link>
      </div>
